...
const AICoach: React.FC = () => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    { role: 'model', text: 'Coach Blaze here. Ready to unleash your inner fighter? Ask me about Boxing, MMA, or our training programs.' }
  ]);
  ...
  return (
    <>
      ...
      <motion.button
        ...
        className={`fixed bottom-8 right-8 z-40 flex h-16 w-16 items-center justify-center rounded-full bg-sanctum-orange text-white shadow-[0_0_30px_rgba(255,36,0,0.5)] transition-opacity duration-300 ${isOpen ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}
      >
          ...
          >
            ...
            <div className="flex items-center justify-between border-b border-white/10 bg-black/50 px-6 py-4">
              <div className="flex items-center gap-3">
                ...
                <h3 className="font-display text-lg font-bold tracking-wider text-white italic">COACH BLAZE</h3>
              </div>
              <button 
            ...
            {/* Messages Area */}
            <div className="flex-1 overflow-y-auto p-6 scrollbar-thin scrollbar-thumb-white/20 bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')]">
                ...
                {isLoading && (
                  <div className="flex justify-start">
                    <div className="flex items-center gap-2 rounded-lg bg-white/5 border border-white/10 px-4 py-3">
                      ...
                      <span className="text-xs text-white/50 uppercase tracking-widest">Training...</span>
                    </div>
                  </div>
                )}
